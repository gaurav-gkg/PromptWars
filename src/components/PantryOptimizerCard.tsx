...
export function PantryOptimizerCard({ optimizer }: PantryOptimizerCardProps) {
  return (
    <section className="bg-[#ffffff] dark:bg-[#1a1c1c] rounded-[24px] shadow-[0_1px_3px_rgba(0,0,0,0.08)] border border-[#bfcaba]/40 p-4 flex flex-col gap-2">
      <div className="flex items-center justify-between border-b border-[#f3f3f3] pb-1">
      ...
      {optimizer.reusedIngredients && optimizer.reusedIngredients.length > 0 ? (
        <div className="flex flex-wrap gap-1 pt-1">
          {optimizer.reusedIngredients.map((ing, idx) => (
            <span 
              key={idx} 
              className="bg-[#cbffc2]/10 dark:bg-[#2e7d32]/10 text-[#0d631b] dark:text-[#a3f69c] border border-[#0d631b]/20 px-2 py-1 rounded-full text-xs font-semibold flex items-center gap-1"
            >
