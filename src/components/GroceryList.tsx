...
export function GroceryList({ initialItems }: GroceryListProps) {
  ...
  return (
    <section className="bg-[#ffffff] dark:bg-[#1a1c1c] rounded-[24px] shadow-[0_1px_3px_rgba(0,0,0,0.08)] border border-[#bfcaba]/40 p-md flex flex-col h-full">
      <div className="flex items-center justify-between mb-md border-b border-[#f3f3f3] pb-sm">
      ...
      <div className="flex-grow space-y-md">
        ...
        {missingItems.length > 0 ? (
          <div>
            <h4 className="font-semibold text-xs text-[#40493d] dark:text-[#bfcaba] uppercase tracking-wider mb-sm">Ingredients Needed</h4>
            <ul className="space-y-sm">
              {items.map((item, idx) => {
                if (item.isAvailable) return null;
                return (
                  <li key={idx} className="flex items-center gap-sm cursor-pointer group">
                    <input
                      type="checkbox"
                      id={`grocery-${idx}`}
          ...
          <div className="text-center py-lg">
            <span className="material-symbols-outlined text-[32px] text-[#0d631b] mb-sm">check_circle</span>
        ...
        {checkedItems.length > 0 && (
          <div className="pt-sm border-t border-[#f3f3f3]/60">
            <h4 className="font-semibold text-xs text-[#707a6c] dark:text-[#bfcaba]/80 uppercase tracking-wider mb-sm">Already in Pantry / Acquired</h4>
            <ul className="space-y-sm">
              {items.map((item, idx) => {
                if (!item.isAvailable) return null;
                return (
                  <li key={idx} className="flex items-center gap-sm opacity-60">
                    <input
                      type="checkbox"
                      id={`grocery-${idx}`}
                      checked={item.isAvailable}
