...
export function SubstitutionCard({ substitutions }: SubstitutionCardProps) {
  return (
    <section className="bg-[#f3f3f3] dark:bg-[#2f3131] rounded-[24px] p-md border border-[#bfcaba]/30">
      ...
      {substitutions.length > 0 ? (
        <div className="flex flex-wrap gap-sm">
          {substitutions.map((sub, idx) => (
            <span 
              key={idx} 
              className="bg-[#ffffff] dark:bg-[#1a1c1c] px-sm py-xs border border-[#bfcaba]/30 rounded-full text-xs text-[#1a1c1c] dark:text-[#ffffff] flex items-center gap-xs shadow-sm"
            >
